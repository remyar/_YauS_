...
void LLADC_Init(uint8_t periphNum)
{
    ...
    case (LL_ADC_PA0):
    {
        if (_allreadyConfigured(GPIO_PIN_0) == false)
        {
            ...
            ADC_channels[ADC_nbChannel] = LL_ADC_PA0;
            ADC_nbChannel++;
        }
        break;
    }
    ...
    case (LL_ADC_PA1):
    {
        if (_allreadyConfigured(GPIO_PIN_1) == false)
        {
            ...
            ADC_channels[ADC_nbChannel] = LL_ADC_PA1;
            ADC_nbChannel++;
        }
        break;
    }
...
uint32_t LLADC_GetValue(uint8_t channel)
{
